...
# sampling 30 individuals' height in the weight-height database with replacement. 
#Note that sampling with replacement is used in bootstrap, and without replacement to give a permutation
wh_sample_height <- sample(wh$height, 30, replace = TRUE)
# set mean as function to be bootstrapped, since mean is a built-in method, this stop could be omitted...
theta <- function(x){mean(x)}             
# 100 bootstraps of the sample mean
result <- bootstrap(wh_sample_height, 100, theta)
# the bootstrap values of theta rounded to two decimal places.
round(mean(result$thetastar), 2)          
